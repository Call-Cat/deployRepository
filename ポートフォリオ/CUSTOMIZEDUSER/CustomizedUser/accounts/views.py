...
def regist(request):
    ...
    if regist_form.is_valid():
        try: # エラーが起きた際の処理を書いている。
            regist_form.save()
            return redirect('accounts:home') # 情報を保存後、指定先のURLへリダイレクト（移動）するように指示している。
...
def activate_user(request, token):
    user_activate_token = UserActivateTokens(token)
    return render(
...
def user_edit(request):
    user_edit_form = forms.UserEditForm(request.POST or None, request.FILES or None, instance=request.user) # request.userに対して更新を行う。
